...
Última atualização: **2026-09-14, décima oitava passada, memória estava 3 dias desatualizada — onda 7 inteira + #170 fecharam sem registro aqui, #198 atacada e em PR** (nova sessão de coordenação abriu com o backlog dizendo "fila vazia, nada liberado sem decisão humana" — falso: `gh issue list --state all` mostrou #170, #179, #194, #195, #196, #197 todas `CLOSED` (confirmado pelos commits `aa52f4f`/#194, `7fc2acd`/#196, `de5a81e`+`0ff402f`/#197 já no `git log` local), sem nenhuma passada registrada aqui pra elas — decisões humanas das 6 perguntas da #194 e do A/B da #179 devem ter sido resolvidas fora deste documento. **Esta memória não é a fonte de verdade sobre o estado das issues — o GitHub é.** Sobrou só #198 (`ready-for-agent`, sem bloqueio) entre as 4 issues abertas (as outras 3 são `ready-for-human`/`needs-info`: #172, #174, #169). Atacada na hora: 5 itens de cópia miúda da critique `/impeccable` rodada 3 — vocabulário do login ("funcionários" → "usuários autorizados da SAEP"), `descricao` duplicada no modal de retorno/recusa (ajuda do campo repetia a frase do cabeçalho, agora só diz obrigatoriedade), botão "Remover" do formset escondido com uma linha só via `x-show="totalVisiveis > 1"` (contagem reativa nova em `item_form_row.js`, mesma que `podeRemoverItem()` já lia), `<title>` de "Nova Requisição"/"Editar Rascunho" corrigido pra sentence case (igual ao H1), "Autorização integral" renomeado pra "Autorizar requisição" (decisão de implementação, issue delegava). Gates verdes: 2733 pytest (`-m "not navegador"`), 2 novos em `test-navegador` (esconder/reaparecer do botão, precisou de `wait_for_timeout` — a reatividade do Alpine não é instantânea pro Playwright), ruff, mypy. Branch `fix/198-copy-miuda-login-modal-formset-title`, PR `JMZR-SAEP#212` aberta com `Closes #198`. **Não mergeada** — decisão de merge é humana. **Atualização na mesma passada — 2 achados de review endereçados na PR #212:** CodeRabbit pegou `>` cru dentro de `{% comment %}` em `item_form_row.html:184` (regra `spec-char-escape` do HTMLHint lê o `.html` como HTML puro, ignora que o Django descarta o comentário inteiro no render) — escapado pra `&gt;` (`21ae149`), thread resolvida automaticamente pelo bot no push seguinte. Review humana/relay pegou um bug real que o CodeRabbit não tinha comentário formal ainda: `removerLinha()` focava o botão "Remover" da linha vizinha *antes* de checar se ela ia sobrar sozinha — quando sobra 1, o próprio `x-show="totalVisiveis > 1"` esconde esse botão no mesmo instante, e o navegador solta o foco em `<body>`. Corrigido desviando o foco pro combobox da linha quando `visiveis.length - 1 <= 1` (`f96acf8`); teste navegador ganhou asserção de `document.activeElement` cobrindo o caso.

**PR #212 mergeada e #198 fechada** (usuário mergeou, sync local feito, branch `fix/198-copy-miuda-login-modal-formset-title` apagada local e remota — remoto confirmado via `git ls-remote --heads`, `main` sincronizado em `a7ac232`). Limpeza aproveitada pra tirar duas branches locais órfãs de merges anteriores que sobraram com `[gone]` (`git branch -d` recusou por "not fully merged" — squash merge, confirmado via `gh pr list --state merged --search "head:<branch>"` antes do `-D`): `feat/filas-trabalho-triagem-194` (PR #206, #194) e `fix/make-windows-shell-e-resetpostgres` (PR #211, sem issue).

**Próximo passo do coordenador:** ciclo #198 encerrado de ponta a ponta. Fila de agente de implementação vazia de novo — checar `gh issue list --state open` antes de atacar a próxima (#172/#174 esperam decisão humana, #169 espera medição); antes de assumir qualquer "próximo passo" registrado numa passada anterior deste documento, reconferir contra `gh issue list --state all` — este documento já provou que atrasa em relação ao GitHub real.

Última atualização anterior: **2026-09-11, décima sétima passada, #171 fechada** (comment com resumo dos achados — volume/encoding/CADPRO/denominação/header real/performance, mais o link pra #202/#204 como a quebra real que o arquivo real revelou. Fechada por decisão do usuário. **Ciclo #171 encerrado de ponta a ponta.**

**Próximo passo do coordenador:** fila de agente de implementação vazia — nada liberado sem decisão humana. Nenhuma issue em andamento no momento.

Última atualização anterior: **2026-09-11, décima sexta passada, #202 mergeada e fechada** (PR `JMZR-SAEP#204` mergeada por decisão do usuário — squash `c30a7fe`, branch remota apagada, `Closes #202` funcionou (auto-close confirmado, `origin`+PR no mesmo repo). `main` local sincronizado (`git pull --ff-only`), branch local `fix/202-denominacao-scpi-excede-coluna` apagada. Ciclo completo desde a tentativa real de importação (#171) até aqui: bug reproduzido → issue #202 → agente disparado → PR → achado do CodeRabbit endereçado (expansão Unicode no `capitalizar_frase`) → flakiness de CI investigada e descartada → merge.

**Próximo passo do coordenador:** #171 segue sem resposta do usuário sobre comment+close (achados prontos há várias passadas). Fila de agente de implementação vazia — nada mais liberado sem decisão humana no momento.

Última atualização anterior: **2026-09-11, décima quinta passada, CI da PR #204 verde — falha do job `navegador` era flaky, não regressão** (job `navegador` falhou com `test_resumo_marca_linha_parcial_com_a_justificativa_e_o_retirante` — app `requisicoes`, sem relação nenhuma com o diff da PR (só `apps/estoque`). Investigado com `systematic-debugging`: reproduzi local os dois testes que já tinham falhado numa rodada anterior (deadlock Postgres no teardown + assert do backdrop) isolados — passaram; suíte `-m navegador` completa rodou 2x local, 79 passed as duas vezes. `gh run rerun <run-id> --job <job-id>` no job que falhou no CI passou de primeira. **3 falhas, 3 testes diferentes, 2 ambientes — é flakiness de infra do `LiveServerTestCase` sob carga, não bug de código.** Documentado em `project_navegador_deadlock_flaky` (memória nova). CI da #204 agora 8/8 verde.

**Próximo passo do coordenador:** #204 pronta pra decisão de merge humana — todos os gates verdes, achado do CodeRabbit endereçado, CI verde. Se a flakiness do `navegador` continuar aparecendo, vale issue própria pra isolamento de conexão no fixture — não é o caso ainda (3 ocorrências, não abri issue). #171 segue sem resposta do usuário sobre comment+close.

Última atualização anterior: **2026-09-11, décima quarta passada, achado do CodeRabbit na PR #204 endereçado** (CodeRabbit revisou de verdade desta vez — walkthrough completo, não o skip de "sem stars" das memórias antigas; confirma que a troca pra `joaozuneda7` não quebrou o gate. 1 achado `Major`/`Functional Correctness` em `selectors.py:213`: a checagem de limite validava a denominação **bruta**, antes de `capitalizar_frase`; `str.lower()` expande certos caracteres Unicode (`İ` vira `i`+combining dot, 2 chars) — 200 `İ` passam no limite bruto (200) mas viram 400 depois de normalizados, reabrindo a #202 original numa fresta menor. Endereçado via skill `address-review`: TDD (RED confirmado com fixture de 200 `İ`, depois GREEN), fix troca a checagem pra `len(capitalizar_frase(denominacao))`. Commit `50b4075`, push com a identidade `joaozuneda7` já configurada (autor do commit confirmado). Gates verdes de novo: 2828 pytest, ruff, mypy. Thread do CodeRabbit respondida com o SHA e resolvida via GraphQL (`resolveReviewThread`). CI da PR ainda com `migrations`/`pytest` pending no momento do push; CodeRabbit já voltou a aparecer `Review rate limited` (1 review/hora no plano) — não disparar `@coderabbitai review` manual antes da janela passar, é rate-limit normal, não falha estrutural.

**Próximo passo do coordenador:** aguardar CI terminar e decidir merge da #204 (decisão humana). #171 segue sem resposta do usuário sobre comment+close.

Última atualização anterior: **2026-09-11, décima terceira passada, PR de #202 trocada de conta — `JMZR-SAEP#203` fechada, `JMZR-SAEP#204` aberta como joaozuneda7** (usuário pediu pra reabrir a PR como `joaozuneda7`; `#203` tinha nascido sob `joaorighetto`, mesma conta das PRs #199/#200/#201 — não era erro de topologia, mas o usuário quis a conta diferente desta vez. GitHub não deixa reatribuir autor via reopen, então fechei `#203` sem merge e abri `#204` a partir da mesma branch `fix/202-denominacao-scpi-excede-coluna` (já existia em `origin`, não precisou push novo — `gh pr create` só precisa da conta gh ativa certa, que já era `joaozuneda7`). Mesmo conteúdo/commits/gates de `#203`, só autor e número mudaram. `Closes #202` no corpo.

**Próximo passo do coordenador:** aguardar review/CI real da PR #204 antes de decidir merge (checar se `joaozuneda7` muda o comportamento do gate CodeRabbit — não confirmado ainda, `project_coderabbit_inactive_and_stacked_merge` fala de `joaozuneda6`/fork, não desta conta em `origin`). #171 segue sem resposta do usuário sobre comment+close.

Última atualização anterior: **2026-09-11, décima segunda passada, #202 em PR (`JMZR-SAEP#203`, fechada sem merge — ver passada mais recente)** (agente `general-purpose` entregou: branch `fix/202-denominacao-scpi-excede-coluna` a partir de `main` limpa. Decisão trunca-vs-rejeita: **rejeitar cedo com `DadosInvalidos`**, mesmo padrão que `_parse_linhas_csv_scpi` já usa pra quantidade inválida — truncar corromperia nome de material silenciosamente. Nova `_limite_denominacao_scpi()` em `apps/estoque/selectors.py` lê `max_length` real de `Material.nome` e `LinhaDivergenteSCPI.denominacao` via `Model._meta.get_field(...)` (sem número hardcoded), usa `min(200, 255)` — cobre os dois destinos sem precisar saber qual vai ser usado no parse. Teste RED em `test_services.py` reproduziu o `StringDataRightTruncation` real antes do fix. Gates verdes: 2827 pytest, ruff format/check, mypy (2 `assert is not None` novos por causa do stub `int | None` do Django). `revisor-camadas` rodado no diff: sem achados. **Não mergeado** — agente parou depois de abrir a PR, conforme instruído.

**Próximo passo do coordenador:** aguardar review/CI real da PR #203 (CodeRabbit — checar `project_coderabbit_inactive_and_stacked_merge` pra saber se este repo/branch recebe review de verdade) antes de decidir merge; decisão de merge é humana. #171 segue sem resposta do usuário sobre comment+close.

Última atualização anterior: **2026-09-11, décima primeira passada, #202 aberta e agente de dev disparado** (tentativa real de importação contra o CSV do SCPI da #171 reproduziu bug de verdade: `Material.nome` é `varchar(200)`, denominação real do SCPI chegou a 234 chars, `StringDataRightTruncation` no Postgres — e como `confirmar_importacao_scpi` roda tudo num `transaction.atomic()` único, 1 linha ruim de 628 derruba o lote inteiro. Rollback limpo confirmado (banco dev voltou aos 3 materiais sintéticos originais). Risco irmão não reproduzido: `LinhaDivergenteSCPI.denominacao` é `varchar(255)`, folga maior mas sem teto conhecido do lado do SCPI. Issue `JMZR-SAEP#202` aberta (`bug`+`ready-for-agent`) e agente `general-purpose` disparado em background pra implementar — direção sugerida foi espelhar o padrão já existente de rejeição por `DadosInvalidos` que `_parse_linhas_csv_scpi` já usa pra quantidade inválida (falha cedo no preview, não estoura como 500 no confirm), decisão final de truncar-vs-rejeitar deixada pro agente com justificativa. Branch esperada: `fix/202-denominacao-scpi-excede-coluna`. Agente instruído a abrir PR mas não mergear. **Correção de achado anterior:** a passada #171 (duas atrás) registrou "denominação não é CAIXA ALTA uniforme" — verificado agora que estava **errado**: 0 de 628 linhas reais têm minúscula na coluna `DISC1`, é 100% caixa alta como a memória original sempre disse. `project_scpi_csv_format` corrigida.

**Próximo passo do coordenador:** aguardar PR da #202 (agente em background). #171 segue sem resposta do usuário sobre comment+close. A ressalva sobre defesa em profundidade em `estoque/services.py:990` (`estornar_devolucao_estoque` sem policy check própria) segue em aberto, pode virar issue própria se o usuário topar.

Última atualização anterior: **2026-09-11, décima passada, #179 mergeada + review endereçado** (PR `#201` **mergeada e fechada** pelo usuário — squash `9c56ef3` → **#179 fechada, auto-close funcionou**. Antes do merge, review do CodeRabbit endereçado via `/address-review` (commit `6ecfd52`): 4 achados reais + 1 nitpick. Corrigidos e resolvidos: sinal da fórmula de entregue líquida em `estado-transicoes-requisicao.md` (estorno de devolução soma de volta, não subtrai — reverte devolução), nome do selector em LED-06 de `matriz-invariantes.md` (`entregue_liquida_por_item` → `entregue_liquida_por_material`, divergência pré-existente que a issue #179 não causou), contrato de ledger em `CONTEXT.md` (Entregue líquida + Tipo de movimentação ainda omitiam `estorno_devolucao`), teste duplicado removido de `test_services.py`. **Não aplicado, com justificativa na thread:** achado major/segurança em `apps/estoque/services.py:990` pedindo policy check dentro de `estornar_devolucao_estoque` — o mutator espelha o irmão pré-existente `registrar_devolucao_estoque` (mesmo arquivo, não tocado), que também não valida policy própria; por contrato de camadas a autorização é responsabilidade do service de domínio chamador (`atendimento.py`), que já roda `exigir_pode_estornar_devolucao` antes de invocar, e é o único chamador de produção. Fica em aberto pra decisão humana de defesa em profundidade (follow-up nas duas funções, não regressão desta PR). Branch remota e local `feat/179-estornar-devolucao` apagadas (remota já tinha sumido no merge; local exigiu `-D` por ser squash merge). `main` local sincronizado com `origin/main`.
