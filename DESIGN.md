...
Onde há ganho real a buscar é na grade, não no chrome: o cartão do catálogo tem 126px de altura e ainda assim só recebia 2 colunas a 1280px, porque a terceira coluna só entrava em `2xl` (1536px) — e o desktop típico do chefe de setor é 1366 ou 1440, ou seja, ele nunca via três colunas. Três colunas a 1280px dão 42px por registro, mais denso que a tabela e sem truncar nada.
...
**Critério para uma listagem ser "densa"** (objetivo, não caso a caso): medida a 322px de largura de cartão — o que 3 colunas dão a 1280px, `(992 − 2×12) / 3` — o cartão renderiza todo campo em uma linha, sem `truncate`/`line-clamp` ativo e sem `<dd>` quebrado, com altura de conteúdo ≤ ~130px (teto de referência: os 126px do catálogo). Mesmo método do reteste acima. Desqualifica: nome de entidade aninhado que quebra (material dentro de outro registro, beneficiário), quatro ou mais campos de dado, ou valor que não seja curto/numérico. Hoje só `estoque/lista_materiais` (catálogo) passa. Uma segunda listagem que passe no critério entra no **mesmo** fragmento — não se cria um terceiro.
