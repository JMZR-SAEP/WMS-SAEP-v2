...
| REQ-09 | Requisições | Cópia recalcula saldo e não copia autorizado/entregue; origem só pode ser atendida (issue #170: recusar não é mais estado terminal). | Service de cópia. | Copiar atendida; bloquear item sem saldo/divergente. | Crit. 1.13 |
...
| LED-06 | Ledger | Entregue líquida = `−Σ delta_fisico` dos tipos `consumo`/`devolucao`/`estorno_requisicao` do item; leitura pura, sem lock próprio. | Selector `entregue_liquida_por_item`. | Sem devolução, com 1, com múltiplas; item de outra requisição gera erro; selector não trava. | ADR-0015 |
