...
| USR-04 | Setores | Todo setor operacional ativo possui um chefe ativo; `Setor.chefe` apontando para usuário inativo viola o invariante. O chefe pertence ao próprio setor (`chefe.setor_id == setor.id`), inclusive depois da designação. | Model/service; validação em alteração, na desativação de usuário e no remanejamento de lotação (`remanejar_usuario` bloqueia a saída de chefe sem substituto). | Criar com chefe ativo; bloquear setor ativo sem chefe ativo; bloquear troca de `User.setor` de quem chefia setor. | Backlog ACE-002 |
...
| PER-05 | Permissões | Superusuário tem permissões totais, incluindo administração, consulta ampla e operações de negócio/estoque. **Ressalva:** não inclui mutação direta pelo admin de `ItemRequisicao`, `TimelineRequisicao` e `SaldoEstoque` — models derivados da máquina de estados e do ledger, que mudam apenas por service. A leitura desses models e todas as ações de negócio permanecem totais. | Policy/admin/service. | Permitir ação técnica; permitir ação operacional; permitir consulta ampla; negar add/change/delete no admin dos três models derivados. | Crit. 11.6; #105 |
...
| EST-07 | Estoque | Divergência crítica: físico < reservado. Estado de domínio válido, não erro: nenhuma operação é bloqueada por criá-lo. Duas origens — importação SCPI e saída excepcional —, ambas obrigadas a avisar as requisições `autorizada` afetadas com timeline `atualizacao_estoque_relevante` + notificação `divergencia_estoque`, na mesma transação da mutação. | Marcador/recalculo no service; hook de aviso injetado pela view. | Importação reduz físico e marca divergência; saída excepcional que rebaixa o físico abaixo do reservado é permitida e gera um evento agregado por requisição. | Crit. 7.3; issue #111 |
...
| LED-01 | Ledger | Toda mutação de `SaldoEstoque` pelos services retrofitados por ADR-0015 gera `MovimentacaoEstoque` na mesma transação; o bootstrap inicial do SCPI (`confirmar_importacao_scpi`) fica fora do ledger nesta fase. | Helper `_registrar_movimentacao` nos services retrofitados. | Reserva, liberação, consumo, saída e estorno de saída geram movimentação; saldo nunca muda sem ela. | ADR-0015 |
| LED-02 | Ledger | `Σ delta_fisico` e `Σ delta_reservado` por `(estoque, material)` reconciliam com `saldo_fisico` e `saldo_reservado`. | Dois deltas assinados por linha. | Reconciliação por coluna após sequência de movimentos. | ADR-0015 |
| LED-03 | Ledger | Movimentação tem exatamente uma origem (`requisicao` XOR `saida_excepcional`) e `tipo` coerente com a origem. | `CheckConstraint`. | Bloquear sem origem, com duas origens, e `tipo`×origem incoerente. | ADR-0015 |
| LED-04 | Ledger | Movimentação não tem ambos os deltas zero. | `CheckConstraint`. | Bloquear `delta_fisico=0 AND delta_reservado=0`. | ADR-0015 |
| LED-05 | Ledger | Ledger é append-only (imutável após criação). | Override `save`/`delete` (app-level). | Bloquear update e delete de linha existente. | ADR-0015 |
| LED-06 | Ledger | Entregue líquida = `−Σ delta_fisico` dos tipos `consumo`/`devolucao`/`estorno_requisicao`/`estorno_devolucao` do item; leitura pura, sem lock próprio. | Selector `entregue_liquida_por_material`. | Sem devolução, com 1, com múltiplas, com estorno de devolução; item de outra requisição gera erro; selector não trava. | ADR-0015 |
| LED-07 | Ledger | Devolvida líquida = `Σ delta_fisico` dos tipos `devolucao`/`estorno_devolucao` do item; leitura pura, sem lock próprio; limita a quantidade estornável de uma devolução. | Selector `devolvida_liquida_por_material`. | Sem estorno, com 1, com múltiplas; estorno maior que devolvida líquida é bloqueado. | ADR-0015; issue #179 |
