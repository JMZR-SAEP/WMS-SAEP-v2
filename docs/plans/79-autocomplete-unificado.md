...
- **`initialLabel` dentro de literal JS**: nome de beneficiário/material pode conter aspas ou acentos — usar `|escapejs` no valor interpolado dentro de `x-data="autocomplete({...})"` para não quebrar o parser JS (o código atual já teria esse risco latente; o partial genérico deve reforçar isso já que passa a ser o único ponto de entrada).
